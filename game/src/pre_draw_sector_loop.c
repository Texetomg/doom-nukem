...
/*   Updated: 2019/11/09 17:36:41 by bfalmer-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init(t_pds_a *pds_a, t_pds *pds)
{
	...
	some = (pds->i - pds_a->for_draw.window.x1) /
	(pds_a->for_draw.window.x2 - pds_a->for_draw.window.x1);
	pds->yt_window = pds_a->for_draw.window.y1t +
	(pds_a->for_draw.window.y2t - pds_a->for_draw.window.y1t) * some;
	pds->yb_window = pds_a->for_draw.window.y1b +
	(pds_a->for_draw.window.y2b - pds_a->for_draw.window.y1b) * some;
	pds->yt_wall = pds_a->for_draw.wall.y1t +
	(pds_a->for_draw.wall.y2t - pds_a->for_draw.wall.y1t) * some;
	pds->yb_wall = pds_a->for_draw.wall.y1b +
	(pds_a->for_draw.wall.y2b - pds_a->for_draw.wall.y1b) * some;
	if (pds->yt_wall < 0)
		pds->k = 0;
	else
		pds->k = pds->yt_wall;
	...
	pds_a->y1 + pds->a * pds_a->x2 / pds_a->y2) /
	((1 - pds->a) / pds_a->y1 + pds->a / pds_a->y2);
	if (pds->x > 0)
...
static void	check_grid2(t_pds_a *pds_a, t_pds *pds)
{
	if (pds->y > 0)
	...
	{
		pds->color = (pds_a->bars->pixels)
		[(int)pds->y * pds_a->bars->w + (int)pds->x];
		if (pds->color != 0)
			((int *)pds_a->game->screen->pixels)
			[pds->k * pds_a->game->screen->w + pds->i] =
			ft_bright(pds->color, pds_a->bright);
	}
}
...
static void	check_grid(t_pds_a *pds_a, t_pds *pds)
{
	if (pds_a->grid == 1)
	{
		...
		pds->a * pds_a->x2 / pds_a->y2) / ((1 - pds->a) /
		pds_a->y1 + pds->a / pds_a->y2);
		if (pds->x > 0)
